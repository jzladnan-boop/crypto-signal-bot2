...
6) حماية بعد الشراء أضيق من العادي: مضاعف ATR للستوب أصغر (تقبّل أقل للتذبذب
   العكسي، لأننا أصلاً بمنطقة ضعف)، مع سقف صلب أضيق.
...
class MeanReversionParallel:
    def __init__(self, client, notify_fn=None, config_fn=None, symbols_fn=None, **config_overrides):
        ...
        بأول كل دورة. ⚠️ ملاحظة: هاي الاستراتيجية بطبيعتها أضيق بالمخاطرة من باقي
        الاستراتيجيات، فلو انمرر config_fn وحدث القيم لأرقام البوت الأساسي العادية
        (أوسع)، ينفع تتأكد إنها لسا مناسبة لمنطقة "شراء الضعف" — أو ما تمرر config_fn
        أصلاً وتخليها تعتمد قيمها الخاصة الأضيق دايماً.
