...
class TrendStochParallel:
    ...
    def scan_for_entry(self):
        ...
        # فحص اتساع السوق أول شي — طلب واحد بس لكل دورة، على نفس قائمة الـ151
        # عملة يلي صياد بيراقبهم (مو نطاق البحث الأوسع لهاي الاستراتيجية)
        # حتى يكون القياس متّسق مع تقارير صياد. لو أغلب السوق نازل (24 ساعة)،
        # نوقف كل محاولة دخول هالدورة بغض النظر عن قوة أي إشارة فردية —
        # تجنّب "السباحة عكس التيار" (زي صفقة ENSO يلي خسرت بيوم كان 81%
        # من السوق أحمر حسب صياد).
