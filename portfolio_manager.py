...
================================================================
البوت عنده هلق 3 "مشترين" مستقلين ممكن يشتغلوا بنفس اللحظة على نفس الحساب:
  1) البوت الأساسي (حلقة الفحص الرئيسية بـ crypto_signal_bot.py)
  2) Range Trading (range_trading_btc.py)
  3) Trend+Stoch الموازية (trend_stoch_parallel.py)

المشكلة: لو اثنين منهم قرروا يشتروا نفس العملة بنفس الوقت (وارد فعلياً، لأنه
فيه تداخل بالعملات بين سلة Range Trading وسلة Trend+Stoch والبوت الأساسي)،
بيصير تضارب حقيقي — أخطرها إنه بعض دوال البيع (Range Trading/Trend+Stoch)
صارت تبيع "كامل الرصيد المتاح" (لتنظيف الغبار)، فلو استراتيجية باعت، ممكن
تبيع رصيد استراتيجية تانية معاها بالغلط.
